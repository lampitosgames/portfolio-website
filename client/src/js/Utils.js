...
const ccCollision = (_sp1, _sp2) => {
    let p1 = _sp1.pos,    p2 = _sp2.pos;
    let r1 = _sp1.radius, r2 = _sp2.radius;
    let pTOp = p2.clone().subtract(p1);
    let rPLUSr = r1 + r2;
    if (pTOp.length() >= rPLUSr) {
        return null;
    } else {
        let diff = rPLUSr - pTOp.length();
        pTOp.normalize();
        return new Vector(0.5 * pTOp.x * diff, 0.5 * pTOp.y * diff);
    }
}
...
const shapeCollision = (_a1, _a2) => {
    ...
    for (let a = 0; a < axes1.length; a++) {
        let range1 = sProject(_a1, axes1[a]), range2 = sProject(_a2, axes1[a]);
        let ov = rangeIntersect(range1[0], range1[1], range2[0], range2[1])
        if (ov <= 0) { return null; }
        if (ov < overlap) { overlap = ov; smallest = axes2[a]; }
    }
    for (let a = 0; a < axes2.length; a++) {
        let range1 = sProject(_a1, axes2[a]), range2 = sProject(_a2, axes2[a]);
        let ov = rangeIntersect(range1[0], range1[1], range2[0], range2[1])
        if (ov < 0) { return null; }
        if (ov < overlap) { overlap = ov; smallest = axes2[a]; }
    }
...
let Utils = {getViewport, clamp, rangeIntersect, randomRange, randomVec, randomInt, ccCollision, shapeCollision};
